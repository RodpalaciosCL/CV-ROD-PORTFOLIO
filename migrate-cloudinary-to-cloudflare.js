...
const config = require('./cloudflare-config');

// Lista de todas las URLs de Cloudinary encontradas
...
// Función para subir a R2 (para videos y archivos)
async function uploadToR2(filePath, filename) {
  ...
  if (config.R2_DOMAIN && config.R2_DOMAIN !== 'https://your-domain.com') {
...
async function migrateAssets() {
  console.log('🚀 Starting Cloudinary to Cloudflare migration...\n');
  ...
  try {
    ...
    for (let i = 0; i < CLOUDINARY_URLS.length; i++) {
      const url = CLOUDINARY_URLS[i];
      console.log(`[${i + 1}/${CLOUDINARY_URLS.length}] 📥 Downloading: ${url}`);
      ...
      // Determinar si es imagen, video o archivo
      const ext = path.extname(filename).toLowerCase();
      let newUrl;
      ...
      if (['.jpg', '.jpeg', '.png', '.webp'].includes(ext)) {
        ...
        newUrl = `${config.IMAGES_DOMAIN}/${config.CLOUDFLARE_ACCOUNT_ID}/${imageId}/public`;
    ...
    console.log('📋 Check migration-mapping.json for the URL mapping');
